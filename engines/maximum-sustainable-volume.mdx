...
- **Sharding**: 100–200 shards (typical from shard-clob.md and clob-system-design.md), each handling O(log n) tick iteration + ReClamm glide in &lt;10 ms per swap.
...
- **Full activation**: 200+ shards, aggressive caching (in-memory + transient virtual states), SIMD/vectorized batching in reclamm_math.go, and CLOB-hybrid routing for &gt;80% of volume.
...
To assert the bound rigorously (stepping back like IMO problem decomposition):
