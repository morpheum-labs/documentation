...
Imagine the current virtual price is far below the target:

```
Target price ───────┐
                    │
Current virtual price is here ───┐
                                 │
                                 ▼           time ──►
                                 Gradually gliding up
```

The speed is fastest at the beginning (large deviation)  
→ becomes slower and slower as it approaches the target
...
See the internal ReClamm design docs and `pkg/modules/clamm/math/reclamm_math.go` for implementation details.
